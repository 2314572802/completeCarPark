    ...
    <el-main>
      ...
      <el-table :data="tableData" style="width: 100%" max-height="450">
        ...
        <el-table-column
          ...
          width="210"
        ></el-table-column>
        <el-table-column fixed prop="parkType" label="车位类型" width="210">
      ...
      <el-dialog title="新增车辆" :visible.sync="insert">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="freeCarMsgForm('form')"
            >确 定</el-button
          >
        </div>
      </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleSizeChange(val) {
      ...
      this.freeCarMsg(this.pageCode, val);
    },
    handleCurrentChange(val) {
      this.freeCarMsg(val, this.pageSize);
    },
    freeCarMsg(pageCode, pageSize) {
      let pageMsg = new URLSearchParams();
      pageMsg.append("pageCode", pageCode);
      pageMsg.append("val", pageSize);
      this.axios({
        method: "get",
        url: "http://localhost:8088/carpark/queryFreeCarInfo?" + pageMsg,
        data: pageMsg,
      }).then((response) => {
    ...
    conditionQuery() {
      let parkId = new URLSearchParams();
      parkId.append("parkId", this.select);
      this.axios({
        method: "get",
        url: "http://localhost:8088/carpark/queryFreeCarMsgByParkId?" + parkId,
        data: this.userName,
      }).then((response) => {
        console.log(response.data);
    ...
    freeCarMsgForm() {
      let freeCarMsg = new URLSearchParams();
      freeCarMsg.append("parkId", this.form.parkId);
      freeCarMsg.append("parkType", this.form.parkType);
      this.axios({
        method: "post",
        url: "http://localhost:8088/carpark/insertFreeCarMsg",
        data: freeCarMsg,
      }).then((response) => {
        console.log(response.data);
        if (response.data === true) {
          ...
          this.$message.error("新增车辆时所选车位类型不符!请选择自由车位！");
